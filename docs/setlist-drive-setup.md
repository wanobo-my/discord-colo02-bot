...
| 役割 | アカウント | 何をするか |
|---|---|---|
| **インフラ側** | `morita.yuto.0031@gmail.com`（個人・2TB） | Google Cloud プロジェクト `colo02-bot` を持つ。OAuth クライアントの発行元 |
| **データ側** | `color.music370@gmail.com`（団体） | 保存先フォルダの所有者。**認可を行い、ファイルの所有者になる** |

この 2 つが別々でよい理由は、**「アプリの身分証をどこで発行するか」と「誰の権限で Drive に書くか」が独立している**ためです。OAuth クライアントは `colo02-bot` プロジェクトが発行し、それを使って `color.music370` が「自分の Drive への書き込みを許可する」という構図になります。

**曲目リストのファイル所有者は `color.music370` になります。** 活動記録は団体に帰属させ、フォームやその他の共有ファイルと同じ場所にまとめるためです。

### 手順ごとのログインアカウント

| ステップ | ログインするアカウント |
|---|---|
| 1. 保存先フォルダの作成 | **color.music370** |
| 2〜4. Google Cloud の設定 | **morita.yuto.0031** |
| 5. リフレッシュトークンの取得（認可） | **color.music370** |
...
1. Drive に保存先フォルダを作り、フォルダ ID を控える（color.music370）
2. 既存プロジェクト `colo02-bot` で Drive API が有効か確認する（morita.yuto.0031）
3. OAuth 同意画面を設定し、**「本番環境」に公開する**（morita.yuto.0031）
4. OAuth クライアント（デスクトップアプリ）を作る（morita.yuto.0031）
5. `.env` に設定してリフレッシュトークンを取得する（認可は color.music370）
6. 動作確認する
7. Koyeb の環境変数に登録する
...
> ログインするアカウント：**color.music370@gmail.com**

1. [Google ドライブ](https://drive.google.com/) を `color.music370` で開く
2. 曲目リストを置きたい場所に、任意の名前でフォルダを作る（このフォルダの中に `#記録_曲目リスト/2026/...` が自動生成されます）
...
> ログインするアカウント：**morita.yuto.0031@gmail.com**（ここからアカウントが変わります）
...
1. [Google Cloud Console](https://console.cloud.google.com/) を `morita.yuto.0031` で開く
...
> ログインするアカウント：**morita.yuto.0031@gmail.com**（引き続き Cloud Console 側）

> ⚠️ **ここが一番の要注意ポイントです。**
> 公開ステータスを「テスト」のままにすると、**リフレッシュトークンが7日で失効し、毎週botが止まります。**
> 必ず「本番環境」に公開してください。
>
> 今回は**認可するアカウント（color.music370）がプロジェクトの所有者と別**なので、
...
5. テストユーザーの欄があれば、**`color.music370@gmail.com`** を追加する
...
> ⚠️ **ここで必ず `color.music370@gmail.com` としてログインしてください。**
> ここで認可したアカウントが、以後 bot が書き込む Drive の持ち主になります。
> 個人アカウント（morita.yuto.0031）で許可してしまうと、ファイルがそちらに保存されます。
...
現在の構成は「**データは団体（color.music370）、インフラは個人（morita.yuto.0031）**」という状態です。
